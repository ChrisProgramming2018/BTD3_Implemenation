...
def evaluate_policy(policy, writer, total_timesteps, args, env, episode=2):
    """
    ...
    path = mkdir("",str(total_timesteps) + "/")
    size = args.size
    print(path)
    avg_reward = 0.
    ...
    for s in seeds:
        ...
        for step in range(args.max_episode_steps):
            ...
            frame = cv2.imwrite("{}/wi{}.png".format(total_timesteps, step), np.array(obs))
            #cv2.waitKey(10)
